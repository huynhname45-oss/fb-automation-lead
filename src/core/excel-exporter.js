...
/**
 * Creates Excel Workbook populated with lead data matching template_import_lead.xlsx
 */
export async function createExcelWorkbook(posts = [], customConfig = {}) {
...
/**
 * Export qualified leads to Excel matching the exact format of template_import_lead.xlsx (File on disk)
 */
export async function exportToExcel(posts = [], filename, customConfig = {}) {
  try {
    await fs.mkdir(EXPORTS_DIR, { recursive: true });
    const workbook = await createExcelWorkbook(posts, customConfig);
...
/**
 * Export qualified leads to Excel Buffer in RAM without writing any file to server disk
 */
export async function exportToExcelBuffer(posts = [], customConfig = {}) {
  try {
    const workbook = await createExcelWorkbook(posts, customConfig);
    const buffer = await workbook.xlsx.writeBuffer();
    return buffer;
  } catch (error) {
    logger.error({ err: error }, 'Failed to export Excel buffer');
    throw error;
  }
}
